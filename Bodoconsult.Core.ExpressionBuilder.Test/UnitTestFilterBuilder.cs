...
using Bodoconsult.Core.ExpressionBuilder.Generics;
using Bodoconsult.Core.ExpressionBuilder.Test.Helpers;
...
{
	[TestFixture]
	public class UnitTestFilterBuilder
	{
        private readonly List<Person> _people;
		
		public List<Person> People => _people;

        public UnitTestFilterBuilder()
        {
            var company = new Company { Name = "Back to the future", Industry = "Time Traveling Agency" };

            _people = new List<Person>
            {
                new Person { Name = "John Doe", Gender = PersonGender.Male, Birth = new BirthData { Date = new DateTime(1979, 2, 28), Country = "USA" }, Employer = company },
                new Person { Name = "Jane Doe", Gender = PersonGender.Female, Birth = new BirthData { Date = new DateTime(1985, 9, 5), Country = " " } },
                new Person { Name = "Wade Wilson", Gender = PersonGender.Male, Birth = new BirthData { Date = new DateTime(1973, 10, 9), Country = "USA" } },
                new Person { Name = "Jessica Jones", Gender = PersonGender.Female, Birth = new BirthData { Date = new DateTime(1980, 12, 20), Country = "USA" } },
                new Person { Name = "Jane Jones", Gender = PersonGender.Female, Birth = new BirthData { Date = new DateTime(1980, 12, 20), Country = "USA" } },
                new Person { Name = "Fulano Silva", Gender = PersonGender.Male, Birth = new BirthData { Date = new DateTime(1983, 5, 10), Country = "BRA" }, Employer = company },
        ...
        [TestCase(TestName = "Build expression from a filter with simple statements")]
        public void BuilderWithSimpleFilterStatementsSuccess()
        {
            var filter = new Filter<Person>();
            filter.By("Name", Operation.EndsWith, "Doe").Or.By("Gender", Operation.EqualTo, PersonGender.Female);
            var people = People.Where(filter).ToList();
            var solution = People.Where(p => p.Name.Trim().ToLower().EndsWith("doe") ||
                                             p.Gender == PersonGender.Female);
            Assert.That(people, Is.EquivalentTo(solution));
        ...
        [TestCase(TestName = "Build expression from a filter with complex statements: no complex filters possible (supports no brackets for logical expressions)")]
        public void BuilderWithComplexFilterStatementsNoSuccess()
        {
            var filter = new Filter<Person>();
            filter.By("Name", Operation.EndsWith, "Doe").And.By("Gender", Operation.EqualTo, PersonGender.Female)
                .Or.By("Gender", Operation.EqualTo, PersonGender.Male);
            filter.By("Name", Operation.EndsWith, "Jones");


            var people = People.Where(filter);
            var solution = People.Where(p => p.Name.Trim().ToLower().EndsWith("doe") && (p.Gender == PersonGender.Female || p.Gender == PersonGender.Male) ||
                                              p.Name.Trim().ToLower().EndsWith("jones"));
            //Assert.That(people, Is.EquivalentTo(solution));

            Assert.AreNotEqual(solution.Count(), people.Count());
        ...
        [TestCase(TestName = "Build expression from a JSON serialized and then deserialized filter with property chain filter statements")]
        public void BuilderWithPropertyChainFilterStatementsJson()
        {
            // Arrange
            var filter = new Filter<Person>();
            filter.By("Birth.Country", Operation.EqualTo, "usa", value2: default, FilterStatementConnector.Or);
            ...
            // Act
            // Act: Serialize
            var json = JsonHelper.JsonSerialize(filter);

            // Act: Deserialize
            var filter1 = JsonHelper.JsonDeserialize<Filter<Person>>(json);

            // Assert
            var people = People.Where(filter1);
            var solution = People.Where(p => (p.Birth != null && p.Birth.Country != null && p.Birth.Country.Trim().ToLower().Equals("usa")) ||
                                             (p.Birth != null && p.Birth.Date <= new DateTime(1980, 1, 1)) ||
                                             p.Name.Trim().ToLower().Contains("doe"));
